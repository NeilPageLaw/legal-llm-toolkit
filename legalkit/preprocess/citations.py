...
class CitationParser:
    ...
    US_COURT_YEAR = re.compile(r"\(([^)]*?(?:Cir\.|Ct\.|Dist\.).*?)\s*(\d{4})\)", re.IGNORECASE)

    # EU case numbers: Case C-123/24 or Case T-123/24
    EU_CASE = re.compile(r"Case\s+([CT])-(\d+)/(\d{2})", re.IGNORECASE)
    ...
    def _find_case_name(self, text: str, citation_pos: int) -> str | None:
    ...
    def _find_paragraph(self, text: str, citation_pos: int) -> str | None:
    ...
    def extract_all(self, text: str) -> dict[str, list[Citation]]:
        ...
        result = {"cases": [], "legislation": [], "other": []}
