...
class LegalMetrics:
    ...
    LEGAL_TERMS = {
        "uk": {
            "claimant", "defendant", "appellant", "respondent", "tort",
            "breach", "duty of care", "negligence", "statute", "regulation",
            "precedent", "ratio decidendi", "obiter dicta", "judicial review",
            "injunction", "damages", "liability", "indemnity", "warranty",
            "covenant", "consideration", "estoppel", "ultra vires",
        },
        "us": {
            "plaintiff", "defendant", "appellant", "appellee", "tort",
            "breach", "due process", "negligence", "statute", "regulation",
            "precedent", "stare decisis", "dicta", "certiorari", "injunction",
            "damages", "liability", "indemnification", "warranty", "covenant",
            "consideration", "estoppel", "preemption", "standing",
        },
        "eu": {
            "applicant", "defendant", "appellant", "member state", "directive",
            "regulation", "decision", "preliminary ruling", "proportionality",
            "subsidiarity", "direct effect", "supremacy", "infringement",
            "annulment", "preliminary reference", "advocate general",
        }
    }
    ...
    def __init__(self, jurisdiction: str = "uk"):
        ...
        self.legal_terms = self.LEGAL_TERMS.get(
            jurisdiction, 
            self.LEGAL_TERMS["uk"]
        )
    ...
    def evaluate_response(
        ...
        # Citation accuracy
        results["citation_metrics"] = self.evaluate_citations(
        ...
        if reference:
            results["similarity_metrics"] = self.evaluate_similarity(
                response, reference
            )
    ...
    def evaluate_citations(
        ...
        if expected:
            ...
            precision = (
                len(found_set & expected_set) / len(found_set) 
                if found_set else 0.0
            )
            recall = (
    ...
    def _is_valid_citation(self, citation: str) -> bool:
        ...
        return (
            bool(self.UK_CITATION.match(citation)) or
            bool(self.US_CITATION.match(citation)) or
            bool(self.EU_CITATION.match(citation))
        )
    ...
    def _calculate_hedging(self, text: str) -> float:
        ...
        hedging_phrases = [
            ...
            "it appears", "it seems", "arguably", "generally",
            "in some cases", "typically", "usually", "often",
            "subject to", "depending on", "it is possible",
        ]
    ...
    def _calculate_aggregate(self, results: Dict[str, Any]) -> float:
        ...
        # Terminology usage
        term_density = results.get("terminology_metrics", {}).get(
