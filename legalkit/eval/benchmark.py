...
class LegalBenchmark:
    ...
    def evaluate(
        ...
        if generate_fn is None:
            ...
            if model is not None:
                generate_fn = lambda prompt: self._generate(
                    model, tokenizer, prompt
                )
            else:
                raise ValueError(
                    "Provide model_path, model+tokenizer, or generate_fn"
                )
        ...
        for task in self.tasks:
            ...
            task_data = (
                test_data.get(task) if test_data 
                else self._load_test_data(task)
            )
    ...
    def _eval_contract_qa(
        ...
        for sample in tqdm(test_data, desc="Contract QA"):
            ...
            is_correct = any(
                ans.lower() in response_lower for ans in answers
            )
            is_partial = any(
    ...
    def _load_test_data(self, task: str) -> List[Dict]:
        ...
        logger.warning(
            f"No bundled test data for {task}. "
            "Provide custom test_data or use HF datasets."
        )
